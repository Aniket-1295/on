...
function getStyle(offset) {
  ...
  const rotY = sign * Math.min(abs * 43, 82)
  const tx = offset * 258
  const tz = -(abs * abs) * 26
  const scale = Math.max(0.58, 1 - abs * 0.15)
  const opacity = Math.max(0, 1 - abs * 0.29)
...
export default function FeaturesOverview() {
  ...
  const handleMouseEnter = () => {
    setIsPaused(true)
    setTargetProgress(Math.round(progress) % N)
  }
  ...
  return (
    <section className="w-full max-w-7xl z-10 mt-8 mr-auto ml-auto pt-24 pr-6 pb-24 pl-6 relative animate-on-scroll" style={{ animation: 'animationIn 0.8s ease-out 0.1s both', background: 'linear-gradient(155deg, #eef3ff 0%, #f5f8ff 45%, #edfdf7 100%)' }}>
      ...
      <div className="carousel-wrapper relative w-full mb-10">
        <div
          ...
          style={{ height: '390px', perspective: '1500px', perspectiveOrigin: '50% 50%' }}
          onMouseEnter={handleMouseEnter}
          onMouseLeave={handleMouseLeave}
        >
          <div
            ref={trackRef}
            className="carousel-track absolute top-8 left-1/2 w-0 h-80"
            ...
            {INDUSTRIES.map((industry, index) => {
              ...
              return (
                ...
                >
                  <div
                    className="icon-wrap"
                    style={{ background: `linear-gradient(135deg, ${industry.gradient[0]}, ${industry.gradient[1]})` }}
                    dangerouslySetInnerHTML={{ __html: industry.icon }}
                  />
                  <div className="card-title">{industry.title}</div>
                  <div className="card-desc">{industry.desc}</div>
                  <span
                    className="card-tag"
                    style={{ color: industry.tagColor, background: industry.tagBg }}
                  >
                    {industry.tag}
                  </span>
                </div>
      ...
      <div className="flex justify-center items-center gap-10 flex-wrap">
        <div className="flex flex-col items-center gap-1">
          <span className="text-3xl font-extrabold text-slate-900 tracking-tight">20+</span>
          <span className="text-sm text-slate-500 font-medium">Industries Served</span>
        </div>
        <div className="w-px h-10 bg-slate-300"></div>
        <div className="flex flex-col items-center gap-1">
          <span className="text-3xl font-extrabold text-slate-900 tracking-tight">100+</span>
          <span className="text-sm text-slate-500 font-medium">Languages Supported</span>
        </div>
        <div className="w-px h-10 bg-slate-300"></div>
        <div className="flex flex-col items-center gap-1">
          <span className="text-3xl font-extrabold text-slate-900 tracking-tight">&lt;300ms</span>
          <span className="text-sm text-slate-500 font-medium">Response Time</span>
        </div>
        <div className="w-px h-10 bg-slate-300"></div>
        <div className="flex flex-col items-center gap-1">
          <span className="text-3xl font-extrabold text-slate-900 tracking-tight">24/7</span>
          <span className="text-sm text-slate-500 font-medium">Always On</span>
        </div>
      </div>
    </section>
  )
